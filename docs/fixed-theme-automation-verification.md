...
and checks the current local wall-clock time every 30 seconds.
...
- When enabled, the saved schedule is evaluated immediately and then every 30 seconds.
...
3. Confirm the current scheduled theme is applied and the next boundary changes it
   within 30 seconds after the selected minute begins.
